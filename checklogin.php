...
$data = $db->prepare( 'SELECT failed_login, last_login FROM ppbacc WHERE username = (:username) LIMIT 1;' );
...
$tbl_name="ppbacc"; // Table name if you wish to use a variable

$select_sql = "SELECT password, salt, id FROM ppbacc WHERE username=:username;";
...
if($checkpassword==$returnedpassword && $_POST['password']<>'' && $account_locked == false){
	...
	// Reset bad login count
		$data = $db->prepare( 'UPDATE ppbacc SET failed_login = "0" WHERE username = (:username) LIMIT 1;' );
		$data->bindParam( ':username', $user, PDO::PARAM_STR );
		$data->execute();
...
// Update bad login count
		$data = $db->prepare( 'UPDATE ppbacc SET failed_login = (failed_login + 1) WHERE username = (:username) LIMIT 1;' );
		$data->bindParam( ':username', $user, PDO::PARAM_STR );
		$data->execute();
...
// $lockout_time for each attempt.
	$data = $db->prepare( 'UPDATE ppbacc SET last_login = now() WHERE username = (:username) LIMIT 1;' );
	$data->bindParam( ':username', $user, PDO::PARAM_STR );
	$data->execute();
